...
def generate_one_dif(rootdir, patient, layer, slicename, channel, save_dir):
    ...
    list_imgs = []
    for ith_model in range(10):
        current_path = os.path.join(rootdir,  patient, str(ith_model), layer, slicename, str(channel) + '.png')
        input_image = Image.open(current_path)
        list_imgs.append(input_image)

    imageio.mimsave(os.path.join(channel_savedir, 'ev.gif', ), list_imgs, duration =0.5)
...
def convert_all_to_gif(root_dir, png_dir, save_dir):
    ...
    list_patients = ['TCGA_CS_6667_20011105'] #[p[p.rfind("/") + 1 :] for p in patient_dirpath]

    list_models = [str(i) for i in range(100)]
    list_layers = get_layer_names()
...
if __name__ == "__main__":
    ...
    png_dir = 'actmap/' #''/playpen-ssd/jyn/infovis/act_map/'
    save_dir = 'gif/' #'/playpen-ssd/jyn/infovis/gif/'
